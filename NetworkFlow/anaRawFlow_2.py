...
class analyzer:
    ...
    def getDF(self, filename = 'raw_SouthRoute2016.csv'):
        ...
        df.index = ser
        return df
    def getFlappingListFromLog(self,foutname):

    def getFlappingListFromRawFlow(self,foutname):
        infos = []
        info = []
        # record last rows have net flow
        fout = open(foutname,'w')
        fout.writelines('dateIndex,StartT,EndT,startT_sec,endT_sec,duration,interval,netflow,accumulate\n')
        previous = None
        previousstart = None
        previousend = None
        ...
        for index, row in self.dfNet.iterrows():
            ...
            # input should > threshold
                ...
                writeline = '{0},{1},{2},{3},{4},{5},{6},{7},{8}\n'.format(
                        previousstart['date'], previousstart['date'],
                        previousend['date'], previousstart['timesec'],
                        previousend['timesec'], duration,
                        interval,netflow,accumulate
                        )
...
obj.getFlappingListFromRawFlow('flappingFromNetflow_dir.csv')
end = time.time()
